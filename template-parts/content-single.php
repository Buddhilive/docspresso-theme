	...
	<header class="entry-header mb-6">
		...
		<div class="entry-meta text-gray-600 text-sm mb-4">
			...
			$docspresso_post_by = sprintf(
				...
				'<span class="author vcard"><a class="url fn n text-purple-600 hover:text-purple-800" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
			);

			echo $docspresso_posted_on . $docspresso_post_by; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

			if ( has_category() ) {
				echo '<div class="post-categories mt-2 text-purple-600">';
			...
			if ( has_tag() ) {
				echo '<div class="post-tags mt-2 text-purple-600">';
