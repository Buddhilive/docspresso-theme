...
<div id="comments" class="comments-area py-12">
	...
	$args = array(
		...
		'title_reply'        => esc_html__( 'Leave a Comment', 'docspresso-theme' ),
		'title_reply_to'     => esc_html__( 'Leave a Reply to %s', 'docspresso-theme' ),
		'cancel_reply_link'  => esc_html__( 'Cancel reply', 'docspresso-theme' ),
		'label_submit'       => esc_html__( 'Post Comment', 'docspresso-theme' ),
		'comment_notes_before' => '<p class="comment-notes text-sm text-gray-600 mb-6">' . esc_html__( 'Your email address will not be published. Required fields are marked *', 'docspresso-theme' ) . '</p>',
		'comment_notes_after' => '',
		'submit_button'      => '<button name="%1$s" type="submit" id="%2$s" class="submit-btn bg-purple-600 hover:bg-purple-700 text-white font-semibold py-3 px-8 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2">%4$s</button>',
		...
		'fields'             => apply_filters(
			...
			array(
				'author' => '<div class="comment-form-author form-group mb-6">' .
					...
					'<input id="author" name="author" type="text" class="form-control w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200" placeholder="' . esc_attr__( 'Your Name', 'docspresso-theme' ) . '" value="' . esc_attr( $commenter['comment_author'] ) . '"' . ( $req ? ' required' : '' ) . ' />' .
					'</div>',
				'email' => '<div class="comment-form-email form-group mb-6">' .
					...
					'<input id="email" name="email" type="email" class="form-control w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200" placeholder="' . esc_attr__( 'your.email@example.com', 'docspresso-theme' ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) . '"' . ( $req ? ' required' : '' ) . ' />' .
					'</div>',
				'url'   => '<div class="comment-form-url form-group mb-6">' .
					...
					'<input id="url" name="url" type="url" class="form-control w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200" placeholder="' . esc_attr__( 'https://example.com', 'docspresso-theme' ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) . '" />' .
					'</div>',
			)
		),
	);
